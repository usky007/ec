...
class AuthMethod_OAuth_Driver implements AuthMethod_Driver {
	...
	const OPTIONS_KEY_VERSION = 'version';
	const OPTIONS_KEY_API_HEADERS = 'api_headers';
	...
	public function request($callback, $return = false)
	{
		...
		$response = $this->http_request($lib, social::auth_url($this->provider, "Credential", self::REQUEST_TOKEN_URL), "GET", array("oauth_callback" => $callback));
		...
		$url = social::auth_url($this->provider, "Credential", self::AUTHORIZE_URL)."?oauth_token={$token['oauth_token']}";
		if ($return) {
			return $url;
		}
		else {
	...
	public function exchange($request_token, $verifier)
	{
		...
		$response = $this->http_request($lib, social::auth_url($this->provider, "Credential", self::ACCESS_TOKEN_URL), "GET", array("oauth_verifier" => $verifier));
		if ($lib->http_code != 200) {
	...
	public function send(CurlRequest $request, Credential $credential)
	{
		...
		$response = $this->http_request($lib, $request->url, $request->method, $request->parameters , $request->multi);
	...
	protected function http_request($lib, $url, $method, $parameters , $multi = false)
	{
		// save old settings
		$data = array('version' => OAuthRequest::$version, 'headers' => OAuthRequest::$headers);
		
		// override
		if (isset($this->options[self::OPTIONS_KEY_VERSION])) {
			OAuthRequest::$version = $this->options[self::OPTIONS_KEY_VERSION];
		}
		if (isset($this->options[self::OPTIONS_KEY_API_HEADERS]) && !empty($this->options[self::OPTIONS_KEY_API_HEADERS])) {
			foreach ($this->options[self::OPTIONS_KEY_API_HEADERS] as $k => $v) {
				if (is_int($k)) {
					OAuthRequest::$headers[] = $v;
				}
				else {
					OAuthRequest::$headers[$k] = $v;
				}
			}
		}
		
		// make call
		$response = $lib->oAuthRequest($url, $method, $parameters , $multi);
		
		// reset settings
		OAuthRequest::$version = $data['version'];
		OAuthRequest::$headers = $data['headers'];
