...
class Dicentry_Model extends ORM_Cached {
	...
	public function safe_update($val, $lock = null) {
		...
		// update success or not, value changed, delete cache;
		if ($this->_cached) {
	...
	public function find($condition = NULL, $field = NULL, $forupdate = false) {
		...
		if (!$forupdate && $this->_cachable && !$this->loaded() && !$this->cached()) {
            ...
            $this->_cached = $this->get_cache()->set($primary_guid, $this->export_values());
		}
		return $this;
	}
    ...
    {
        // For specified key, result with no record will be cached in find(). We have to delete it anyway.
        $return = parent::save();
        if ($this->_cached) {
	        $this->get_cache()->delete($this->get_guid());
        }
    ...
    public function val_type(&$length = null)
    {
    	foreach ($this->_db->field_data($this->_table_name) as $row)
		{
			if ($row->Field == 'val') {
				$expression = strtolower(trim($row->Type));

				if (($open  = strpos($expression, '(')) !== FALSE)
				{
					// Find closing bracket
					$close = strpos($expression, ')', $open) - 1;

					// Find the type without the size
					$type = substr($expression, 0, $open);
					$length = intval(substr($expression, $open + 1, $close - $open - 1));
				}
				else
				{
					// No length
