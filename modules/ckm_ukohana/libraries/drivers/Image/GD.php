 ...
 * GD Image Driver.
 *
 * $Id: GD.php 329 2011-06-21 03:08:23Z zhangjyr $
 *
 * @package    Image
 * @author     Kohana Team
...
class Image_GD_Driver extends Image_Driver {
	...
	public function resize($properties)
	{
		...
		else if ($properties['master'] === Image::FILL)
		{
			// Change an automatic master dim to the correct type
			$properties['master'] = (($width / $properties['width']) < ($height / $properties['height'])) ? Image::WIDTH : Image::HEIGHT;
		}
	...
	{
        if ($amount === Image_Driver::ROTATE_AMOUNT_AUTO) {
    ...
    protected function auto_rotate(){
        $dg = 0;
        $exif = exif_read_data($this->image['file'],1,true);
        //var_dump($exif);exit;
	...
	public  function addtext($settings)
	{
		foreach($settings as $setting)
		{
			imagefttext ( $this->tmp_image , $setting['size'] , 0, $setting['x'] , $setting['y'] ,
					$this->getColor($setting['color'])  , $setting['font'] ,$setting['text']  );
		}
		return true;
	}

	public function copyimg($settings)
	{
		...
		foreach($settings as $setting)
		{
		    ...
		    $srcobj = $driver->getResource($srcimg);
			if(!isset( $setting['src_w']))
			{
				$setting['src_w']  = imagesx($srcobj);
			}
			if(!isset( $setting['src_h']))
			{
				$setting['src_h']  = imagesy($srcobj);
			}
			imagecopy ( $this->tmp_image , $srcobj, $setting['dst_x'] , $setting['dst_y'] ,
					$setting['src_x'] , $setting['src_y'] , $setting['src_w'] , $setting['src_h']   );
			imagedestroy($srcobj);
		}
		return true;
	}

	public function addRectangle($settings)
	{

		foreach($settings as $setting)
		{

			imagefilledrectangle ($this->tmp_image ,$setting['x1'], $setting['y1'] , $setting['x2'], $setting['y2'],$this->getColor($setting['color'])  );
	...
	protected function getColor($strcolor)
	{
		$strlen = strlen($strcolor);
		$red = $green = $blue = 0;
		if($strlen==7)
		{
			$red = hexdec(substr($strcolor,1,2));
			$green = hexdec(substr($strcolor,3,2));
			$blue = hexdec(substr($strcolor,5,2));
		}
		else if ($strlen == 6)
		{
		...
		return imagecolorexact( $this->tmp_image,$red,$green,$blue);
	}
