 ...
 *
 * $Id: Image.php 329 2011-06-21 03:08:23Z zhangjyr $
 *
 * @package    Image
 * @author     Kohana Team
...
class Image_Core {
	...
	{
		return $this->driver->getResource($this->image);
	}*/
	/**
	 ...
	 * @return  object
	 */
	public function addtext($text,$size,$x,$y,$color,$font)
	{
	...
	public function copyimg(  $src_im ,   $dst_x ,   $dst_y ,   $src_x=0 ,   $src_y=0 ,   $src_w=null ,   $src_h=null)
	{
		$setting =  array(
			'src'=>$src_im,
 			'dst_x'=>$dst_x,
		...
		if(!is_null($src_h))
			$setting['src_h'] = $src_h;
		$this->actions['copyimg'][] = $setting;
		return $this;
	}

	public function addRectangle($x1,$y1,$x2,$y2,$color)
	{
		$this->actions['addRectangle'][] = array(
 			'x1'=>$x1,
 			'y1'=>$y1,
 			'x2'=>$x2,
 			'y2'=>$y2,
	...
	public function save($new_image = FALSE, $chmod = 0644, $keep_actions = FALSE)
	{
		...
		{
			throw new Kohana_Exception("create ".$dir." failed." );
		}
