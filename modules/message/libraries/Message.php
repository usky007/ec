...
class Message {
	...
	function Message() {
		...
		//$this->logconfig = Kohana::config('log');
	}

	function __destruct() {
		if (isset($this->con)) {
			$this->con->disconnect();
			//log_message('info', 'Destroy Message');
		}
	}

	function send($message, $config_item=NULL) {
		/*$mq_enable = config_item('mq_enable');
		if((!$mq_enable['send_msg']) or ($config_item != NULL and $mq_enable[$config_item] === false)) {
			return true;
		}*/

		if (!isset($this->con)) {
			$this->con = new StompConnection("tcp://" . config::item('log.mq_addr') . ":61613");
			//$this->con = new StompConnection("tcp://localhost:61613");
			//echo "do connect";
			$this->con->connect();
			//echo "finish connect";
		}
		...
		//$mq_queue = config_item('mq_queue');
		$mq_queue = config::item('log.mq_queue');
		if (!isset($mq_queue[$config_item])) {
			$queue = $mq_queue["default"];
		} else {
			$queue = $mq_queue[$config_item];
		}
		//print_r($this->logconfig);
	...
	function log_uh($message, $properties, $config_item=NULL)
	{
		$mq_enable = config::item('log.mq_enable');
		if((!$mq_enable['send_msg']) or ($config_item != NULL and !$mq_enable[$config_item])) {
			return true;
		}

		if (!isset($this->con)) {
			$this->con = new StompConnection("tcp://" . config::item('log.mq_addr') . ":61613");
			//$this->con = new StompConnection("tcp://localhost:61616");
			$this->con->connect();
		}
		...
		$content = $this->makeLog($message, $properties);

		// send a message to the queue
		$mq_queue = config::item('log.mq_queue');
		if ( isset($mq_queue["boardcast_uh"]) && !empty($mq_queue["boardcast_uh"]) ) {
			$detination = $mq_queue["boardcast_uh"];
		} else {
			//$detination = "/topic/uutuuUHLog";
			$detination = "/test/recall";
		}
		$con->send($detination, $content, $properties); // message 即上一节中定义的消息结构
	 ...
	 * @return unknown
	 */
	function async_call($url=null, $func , $array) {

		$mq_enable = config::item('log.mq_enable');

		/*if ($mq_enable['async_log']) {
			$log_percent = $this->config['log']['log_percent'];
			$rand = rand(1, 100);
			if ($rand <= $log_percent) {
	...
	private function _do_async_call($url)
	{
		...
		//$url = $this->config["async_host"].$url;
		//echo "[$url]";exit;
		$message = array("action"=>"post","url"=>$url,"info"=>array());
		$content = $this->makeMessage($message);
	...
	function makeMessage_old($msg) {
		$content = '<?xml version="1.0" encoding="utf-8"?><message>';

		if(is_array($msg)) {
			foreach($msg as $k => $v) {
				if($msg['action'] == 'post' && $k == 'info') {
					$value = json_encode($v);
					$content .= '<' . $k . '><![CDATA[' . $value . ']]></' . $k . '>';
				} else {
					$content .= '<' . $k . '>' . $v . '</' . $k . '>';
				}
			}
