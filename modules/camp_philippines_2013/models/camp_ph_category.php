<?php
class Camp_Ph_Category_Model extends ORM_Cached
{
	protected $_primary_key = 'id';
	protected $_table_name = 'Camp_Ph_Category';
	...
	public function getTweets($keyword, $start, $range)
	{
		...
		// $tname = $db->table_prefix().'Camp_Ph_Tweet';

		$db = & Database::instance();
 		$db->from($this->_table_name);
 		$db->join('Camp_Ph_Tweet', array($this->_table_name.'.tweet_id' => 'Camp_Ph_Tweet.tweet_id'), '', "INNER");
 		$db->where(array($this->_table_name.'.keyword' => $keyword));
 		$db->orderby('Camp_Ph_Tweet.created', 'desc');
 		$db->select('*');
	...
	public function reCate()
	{
		...
		if(count($category) > 0)
		{
			$cwcm = new Camp_Ph_Category_Model();
			$cwcm->where('tweet_id >', 0)->delete_all();

			$cwtm = new Camp_Ph_Tweet_Model();
			$cwts = $cwtm->find_all();
	...
	public static function deploy() 
	{
		...
		try {
			$result = $db->query("
					CREATE TABLE  IF NOT EXISTS  `{$db->table_prefix()}Camp_Ph_Category` (
