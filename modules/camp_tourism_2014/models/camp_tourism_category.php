<?php
class Camp_Tourism_Category_Model extends ORM_Cached
{
	protected $_primary_key = 'id';
	protected $_table_name = 'Camp_Tourism_Category';
	...
	public function getTweets($keyword, $start, $range)
	{
		...
		$db = & Database::instance();
 		...
 		$db->join('Camp_Tourism_Tweet', array($this->_table_name.'.tweet_id' => 'Camp_Tourism_Tweet.tweet_id'), '', "INNER");
 		$db->where(array($this->_table_name.'.keyword' => $keyword));
 		$db->orderby('Camp_Tourism_Tweet.created', 'desc');
 		$db->select('*');
	...
	public function reCate()
	{
		...
		if(count($category) > 0)
		{
			$cwcm = new Camp_Tourism_Category_Model();
			$cwcm->where('tweet_id >', 0)->delete_all();

			$cwtm = new Camp_Tourism_Tweet_Model();
			$cwts = $cwtm->find_all();
	...
	public static function deploy() 
	{
		...
		try {
			$result = $db->query("
					CREATE TABLE  IF NOT EXISTS  `{$db->table_prefix()}Camp_Tourism_Category` (
	...
	public static function deploy_1() 
	{
		...
		try {
			...
			$preference->set('tourism-category', json_encode(array()));
