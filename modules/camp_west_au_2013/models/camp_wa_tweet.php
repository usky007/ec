<?php
class Camp_Wa_Tweet_Model extends ORM_Cached
{
	protected $_primary_key = 'tweet_id';
	protected $_table_name = 'Camp_Wa_Tweet';
	...
	public static function deploy() 
	{
		...
		try {
			$result = $db->query("
					CREATE TABLE IF NOT EXISTS `{$db->table_prefix()}Camp_Wa_Tweet` (
	...
	public static function deploy_1()
	{
		...
		try {
			$result = $db->query("
						ALTER TABLE  `{$db->table_prefix()}Camp_Wa_Tweet` CHANGE  `city`  `source` VARCHAR( 50 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT  '来源'
					");
					echo  __CLASS__.' deployed.<br />';
					return 2;
		}
