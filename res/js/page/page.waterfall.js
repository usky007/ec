		...
		if(loadWaterFall){
			...
			$.ajax({
				...
				data: {
                        ...
                        'camp_name' : js_context.camp_name,
						'cateModel' : js_context.cateModel
					  },
				dataType: 'json',
				success:function(data){
					$('.loading').hide();
...
//								console.log(index +' : '+temp_h);
	...
	var addContent = function(tweet, $row)
	{
		...
		{
			currentCon += '<div class="user_fb_source">发送自 <a href="'+tweet.source.link+'" target="_blank">'+tweet.source.name+'</a> </div>';
		}
		currentCon += '</div>';
