...
TopUp = (function() {
  ...
  var transform = function(direction, dimensions, callback) {
	...
	var setDimensions = function(dimensions) {
		// add by Tianium (2011-06-14)
		// checkWidth instead of checkHeight
	  var func = dimensions ? null : checkWidth;
	...
	// add by Tianium (2011-06-14)
	// alternate function to regard width as priority demension.
	var checkWidth = function() {
		if (jQuery("#temp_up").outerWidth() <= jQuery(window).width() - 4) {
			return;
		}
		
		var extraWidth = jQuery("#temp_up").outerWidth() - jQuery("#temp_up .te_content").width(),
			dimensions = {width: jQuery(window).width() - 10 - extraWidth};
		
		if (options.type == "image") {
			dimensions.height = parseInt(options.content.height() * (dimensions.width / options.content.width()), 10);
		}
